...
["email","Correo electrónico","Ejemplo: ejemplo@email.com","email","required"],
...
["email","Email address","Example: example@email.com","email","required"],
...
function savePractice(type){
 ...
 const values={};
 qsa("input,select",box).forEach((el,i)=>{
  const f=fields[i];
  if(f)values[f[0]]=el.value
 });

 try{
  localStorage.setItem(getPracticeKey(type),JSON.stringify(values));
 ...
 if(f[3]==="review"){
  ...
  if(input){
   input.addEventListener("input",()=>{
    state.sim[type][f[0]]=input.value
   })
  }
 }
...
function captureCurrent(type){
 ...
 const f=fields[type==="visa"?state.visaSimStep:state.dSimStep];
