...
function applyLanguage(){
 ...
 if(g){
  const title=qs("#catAccessTitle"),text=qs("#catAccessText"),payTitle=qs("#catPayTitle"),payText=qs("#catPayText"),payBtn=qs("#catPayBtn"),freeTitle=qs("#catFreeTitle"),freeText=qs("#catFreeText"),adminTitle=qs("#catAdminTitle"),userLabel=qs("#catUsernameLabel"),passLabel=qs("#catPasswordLabel"),loginBtn=qs("#catLoginBtn"),closeBtn=qs("#catCloseAdmin"),hint=qs("#catAccessHint"),expiredTitle=qs("#catExpiredTitle"),expiredText=qs("#catExpiredText"),expiredBtn=qs("#catExpiredBtn");
  if(title)title.textContent=t.accessTitle;if(text)text.textContent=t.accessText;if(payTitle)payTitle.textContent=t.payTitle;if(payText)payText.textContent=t.payText;if(payBtn)payBtn.textContent=t.payButton;if(freeTitle)freeTitle.textContent=t.freeTitle;if(freeText)freeText.textContent=t.freeText;if(adminTitle)adminTitle.textContent=t.adminTitle;if(userLabel)userLabel.textContent=t.username;if(passLabel)passLabel.textContent=t.password;if(loginBtn)loginBtn.textContent=t.login;if(closeBtn)closeBtn.textContent=t.close;if(hint)hint.textContent=t.threeTap;if(expiredTitle)expiredTitle.textContent=t.expiredTitle;if(expiredText)expiredText.textContent=t.expiredText;if(expiredBtn)expiredBtn.textContent=t.expiredButton
 }
...
async function validateStoredAccess(){
 ...
 try{
  ...
  saveAccess(token,Number(d.remaining_seconds),sessionStorage.getItem("cat_access_type")||"paid");
  return true
...
async function adminLogin(){
 ...
 try{
  ...
  const d=await r.json().catch(()=>({}));
  if(!r.ok||!d.access_token){setAccessMessage(T[state.lang].wrongLogin,true);return}
  if(!saveAccess(d.access_token,Number(d.expires_in||1080),d.type||"admin")){setAccessMessage(T[state.lang].wrongLogin,true);return}
  hideAccessGate();
  await loadData()
 }catch(e){
