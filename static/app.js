...
async function loadData(){
 ...
 try{
  ...
  else if(r.status===401||r.status===403){expireAccess();return}
...
async function validateStoredAccess(){
 ...
 try{
  ...
  if(!d.ok||Number(d.remaining_seconds)<=0)return false;
...
async function verifyStripeReturn(){
 ...
 try{
  ...
  const d=await r.json().catch(()=>({}));
  if(!r.ok||!d.token)throw new Error(d.detail||"payment");
  if(!saveAccess(d.token,Number(d.expires_in||1080),d.kind||"paid"))throw new Error("token");
...
async function adminLogin(){
 ...
 try{
  ...
  if(!saveAccess(d.token,Number(d.expires_in||1080),d.kind||"admin")){
